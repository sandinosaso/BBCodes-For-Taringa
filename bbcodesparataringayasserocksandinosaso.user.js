...
//var urlscript = 'http://phpcssajaxsoft.byethost11.com/demos/taringa/scripts/';
var urlimg = urlscript+'imagenes/';
//var urlimg = 'http://phpcssajaxsoft.byethost11.com/demos/taringa/scripts/imagenes/';
...
case 'comunidades':
	...
	//--Numeración de comentarios--//
	var n_com_ini = 20 * parseInt($('a[class=here]:first').html()) - 20 || 0;
	var n_com = $('.comment-info .nick');
	for(var i=0;i < n_com.size();i++){
		n_com.eq(i).before('#'+(i+n_com_ini+1)+' - ');
		
		/*
		//PRUEBA - Mostrar Estado Online/Offline del usuario al lado del nick en cada post suyo
		//Esta comentado pues no se me ha ocurrido hacer algo mas eficiente que lo siguiente
		//Hacer pedidos ajax a 'http://www.taringa.net/'+url_perfil_usuario y de ahi tomar el icon (que es un span)
		//del estado del usuario (boton verde o gris) y agregarlo junto al nick.
		//El tema es que se podrian llegar a hacer tantas llamadas ajax como comentarios en la pagina hallan. Esto es 
		//superlento y una chanchada pero no se me ocurre como sino obtener este dato sin pedir toda la pagina, una 
		//alternativa pense podria ser viable es el .load de jquery que permite especificar un css selector de la porcion
		//que queremos cargar de toda la pagina y por lo que dice en la documentación te trae toda la pagina igual
		//asi que seria equivalente a hacer el post/get correspondiente y luego el .find de lo que querramos (de hecho
		//asi es como lo hago).
		//Lo que si se me ocurrio es declarar un array para tener como claves los nick que voy encontrando, de esta
		//forma si tengo mas de un comentario por usuario hago solo un pedido ajax por todos sus comentarios. Esto
		//serviria mucho en paginas con comentarios de pocos usuarios y repetidas pero seria una pesadilla en las
		//que todos los comentarios son de autores distintos.  Igual con esta solución el peor caso sigue siendo 
		//muchas llamadas ajax.

		
		var url_perfil_usuario = n_com.eq(i).attr('href'); //Url del perfil del usuario en Taringa 
		
		//Traigo el estado Online/Offline del usuario
		$.ajax({
				type: 'GET',
				url: 'http://www.taringa.net'+url_perfil_usuario,
				dataType: 'text',
				data: '',
				success: function(h){
					var respuesta = $('<div id="respuesta">'+h+'</div>').find('div[class="title-w clearfix"] span:first').html();
					n_com.eq(i).after(respuesta);
